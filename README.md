...
## 역할 단위로 폴더 구조를 나누었습니다.

폴더 구조는 기능 중심의 화면 구성과 역할 기반 책임 분리를 결합한 구조로 설계했습니다.
화면별 주요 기능은 `screens` 디렉토리 내에서 도메인 단위로 분리되며, 각 폴더 안에 관련 UI와 로직을 함께 구성해 높은 응집도를 유지합니다. 이로 인해 기능 단위로 코드를 파악하고 유지보수하기 쉬워집니다.

공통으로 사용하는 UI 컴포넌트, 상태 관리, 외부 API 호출 등은 components, store, api 디렉토리로 분리해 앱 전반에서 재사용할 수 있도록 구성했습니다. 각 책임 단위 간의 경계를 명확히 나눠 역할별 모듈화를 강화했습니다.

기능 단위의 유연한 확장과 공통 로직을 재사용하고자 했으며, 사용자 인터랙션 흐름, 내부 상태, 외부 서비스 연동 간의 연결성을 높여 일관된 앱 아키텍처를 구현하고자 했습니다.

```bash
📦src                      # 소쿠리 iOS 앱의 핵심 소스 디렉토리
 ┣ 📂api                   # 백엔드 서버와의 통신 모듈 (크롤링 API)
 ┣ 📂assets                # 정적 리소스 (폰트 및 아이콘 이미지 등)
 ┃ ┣ 📂fonts               # 앱에서 사용하는 커스텀 폰트 파일들
 ┃ ┗ 📂images              # SVG 아이콘 및 이미지 리소스 (가방, 박스, 메뉴 등)
 ┣ 📂components            # 재사용 가능한 공통 UI 컴포넌트 (모달, 헤더, 사이드 메뉴 등)
 ┣ 📂native                # iOS 네이티브 모듈 연동 브릿지
 ┣ 📂screens               # 기능별 화면 구성 디렉토리
 ┃ ┣ 📂ItemList            # 아이템 목록 관련 UI 및 입력 탭 (사용자 아이템, 프리셋)
 ┃ ┣ 📂Simulate            # WebView 기반 3D 시뮬레이터 및 관련 UI
 ┃ ┣ 📜Home.jsx            # 앱 진입 시 보여지는 메인 홈 화면
 ┃ ┗ 📜SimulateScreen.jsx  # 시뮬레이션 탭의 전체 컨테이너 화면
 ┣ 📂store                 # zustand 기반 전역 상태 관리 모듈 (slice 단위)
 ┗ 📜constants.js          # 아이템 프리셋, 고정값 등 상수 정의
...
| 기술 스택 | 설명 |
|-----------|------|
| **React Native** | iOS 앱을 크로스 플랫폼 환경에서 빠른 개발과 배포를 위해 선택했습니다. |
| **Xcode** | 최신 iOS 버전에 맞춰 앱을 빌드하고 테스트하는 데 사용됩니다. 디바이스 최적화와 앱 스토어 배포를 지원합니다. |
| **Zustand** | 전역 상태를 간결하게 관리할 수 있는 경량 상태 관리 라이브러리입니다. 보일러플레이트 없이 유연한 상태 관리가 가능합니다. |
